...
cell_sim_data{1,1}='NatsStadiumFSS';
cell_sim_data{1,2}=38.8730; %%%%%Lat
cell_sim_data{1,3}=-77.0074; %%%%Lon
cell_sim_data{1,5}=10;  %%%%%Height
cell_sim_data{1,6}=0.24; %%%%BW
cell_sim_data{1,7}=0; %%%Double Zero should keep it fixed? (Non-rotating)
cell_sim_data{1,8}=0; %%%Double Zero should keep it fixed? (Non-rotating)
cell_sim_data{1,9}=58; %%%Ant Gain
cell_sim_data{1,11}=-6; %%%I/N dB
cell_sim_data{1,13}=0; %%%FDR dB
...
cell_sim_data{1,22}=12.2; %%An Diameter meters
cell_sim_data{1,23}='WGS F6 (USA 244)';
cell_sim_data{1,24}=176.20;%%%%%Noise Temperature K
cell_sim_data{1,26}='itu_antenna_appendix8_annex3';
cell_sim_data{1,27}=60;%%%%Rx IF bandwidth MHz
cell_sim_data{1,29}=1;%%%%tf_custom_ant_pattern (which means we will save it and pull it).
cell_sim_data{1,30}=3;%%%%x_pol_dB 3dB
...
sample_spacing_km=5;
sim_folder1='C:\Local Matlab Data\7GHz FSS Neighborhood Test Sims'
str_tle_filename=strcat('full_geo_',num2str(rev),'.tle');
...
for base_idx=2:1:num_locations
    ...
    tic;
    [matlab_ele_m]=matlab_elevation_rev1(app,lat_pt,lon_pt);
    toc;
    tic;
    [elevation_m]=elevation_usgs_rev1(app,lat_pt,lon_pt);
    toc;
    horzcat(matlab_ele_m,elevation_m)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
neighborhood_wrapper_rev5_miti_geoplots_pea_custant(app,rev_folder,parallel_flag,tf_server_status,workers,tf_recalculate,tf_rescrap_rev_data)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
